...
碰 UI 时这里多两样东西：访谈里定下来的**结构决策**（哪个状态模型、surface 按什么结构组织），以及跑过 `prototype` 的话，那次裁决的**原型指针**（指向它落的 raw，用 wikilink）。结构已经被一个可点击的东西和一个人的选择定住了，下游发散从下一层高度开始，不重开它。
...
## 7 关联
raw 素材、相关 spec、相关 ADR。跑过 `prototype` 的，把它落的那份 raw 挂在这里——**只挂，不补写**，那份 raw 由 `prototype` 自己写完了。
```

### 碰 UI 的 spec 停在 `等设计冻结`

这次需求碰 UI，`status` 就落 `等设计冻结`，**不落 `在飞`**。这不是"没写完"，是一个真实存在的状态：问题、方案、user story、结构决策都已经定了，缺的只是设计还没收敛成一份可以照着实现的画布。

翻成 `在飞` 是收敛那一段的事——它会用**冻结摘要**小节取代 §4 里的"设计方向"，那时 `to-ticket` 才誊得出设计冻结字段（在此之前它一律落 ⛔）。所以别为了让 spec 看起来"完成"而提前翻状态。

纯后端、埋点、bug fix 不碰 UI 的，照常落 `在飞`，直接进 `to-ticket`。
...
两个产物落完、索引刷完之后，**明说下一环是谁**：

- spec 落了 `等设计冻结` → 下一步是 `uiux-imagine`（设计发散）。告诉人去跑它，不要直接进 `to-ticket`——那样切出来的 UI ticket 只会全是 ⛔。
- spec 落了 `在飞` → 下一步是 `to-ticket`。
