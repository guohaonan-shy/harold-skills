...
- `idea-loop/` — 从 idea 到工程落地的闭环，从一个真实项目（Toeflair）里通用化出来。**9 个 skill**：`grill`（设计树访谈，frontier round）/ `prototype`（一次只回答一个问题的丢弃式原型，`grill` 会在会话内调它）/ `to-spec`（转录 + spec）/ `uiux-imagine`（设计发散：只打开还没定的那一档高度，最低可分辨保真度出变种，只接方向性 note、不打分、不提保真度，准出一份七节方向说明写回 spec，形状由 `scripts/direction-note-check.mjs` 校验）/ `to-ticket`（tracer-bullet 拆卡，守设计冻结闸门）/ `implement`（一卡一 commit，TDD；UI 卡多跑一道测量/像素比对，让「像不像」变成会红的测试）/ `pr-open-review` + `pr-fix-verify`（三轴 review：Correctness / Standards / Spec，永不合并重排）/ `dreaming`（知识库对账）。**设计侧住在这个 plugin 里，不是独立 plugin**：原 `design` plugin 已退役并入，taste/craft references 整体落 `references/design/`（core 法则、DESIGN.md 八节格式、surface / module / component 三档 altitude protocol，加上由原两个 design skill 降级而来的静态 UI 与动效 protocol），全部按需加载、没有一个是入口；确定性 design-lint hook 与其脚本、单测落 `hooks/` 与 `scripts/`；浏览器（headless Playwright MCP）落 `.mcp.json`。**跟本仓库其它 plugin 的约定不同的三处，都是刻意的**：① 不走「pushy description 抵御 undertrigger」那套——`grill` / `uiux-imagine` / `implement` / `dreaming` 四个的 frontmatter 是 `disable-model-invocation: true`，它们是用户显式调用的重活（`uiux-imagine` 每一轮都要人给方向性反应、由人宣布方向定了；`implement` 更是要求一个全新会话，而清上下文只有人能做），不该在无关对话里被动触发；其余五个模型可调，`prototype` 尤其要能被 `grill` 在访谈中途调起；② 第一个带 `workflows/`（Workflow 工具的 `.mjs` 脚本，不是 `skills/`）的 plugin，标准目录结构里已经标成可选项；③ 唯一带 `hooks/` 与 `.mcp.json` 的 plugin。三轴 review 的 Standards 轴读**目标仓库自己**的 `CLAUDE.md` 和 lint/typecheck 配置，不硬编码 Toeflair 那套。详见 `idea-loop/README.md`。
