...
- `idea-loop/` — 从 idea 到工程落地的闭环，从一个真实项目（Toeflair）里通用化出来。**7 个 skill**：`grill`（设计树访谈，frontier round）/ `to-spec`（转录 + spec）/ `to-ticket`（tracer-bullet 拆卡）/ `implement`（一卡一 commit，TDD）/ `pr-open-review` + `pr-fix-verify`（普通 Codex 正确性审查 + 项目规则/Spec 检查，独立验证后跨轴去重，GitHub PR 描述/评论/线程承载 examples 与 Mermaid）/ `dreaming`（知识库对账）。**跟本仓库其它 plugin 的两处约定不同，是刻意的，不是遗漏**：① 不走"pushy description 抵御 undertrigger"那套——这些是显式调用的重活，不该在无关对话里被动触发。`grill`（人做决策的访谈）和 `dreaming`（提议销毁性处置）保留 `disable-model-invocation: true`；`implement` / `pr-open-review` / `pr-fix-verify` 已解锁给调用方，让一张 ticket 能被派出去做到一个已 review 的 PR，解锁的是"谁能调"而不是它们各自的前置（见 `idea-loop/skills/to-ticket/SKILL.md` §6）；② 第一个带 `workflows/`（Workflow 工具的 `.mjs` 脚本，不是 `skills/`）的 plugin，标准目录结构里已经标成可选项。三轴 review 的 Standards 轴读**目标仓库自己**的 `CLAUDE.md` 和 lint/typecheck 配置，不硬编码 Toeflair 那套。详见 `idea-loop/README.md`。
